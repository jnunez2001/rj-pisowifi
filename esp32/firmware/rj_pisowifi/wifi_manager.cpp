...
bool registerVendo() {
  ...
  if (code == 200) {
    ...
    if (config.is_adopted && !status.isEmpty() && !nowAdopted) {
      ...
      // silently leaving setup mode again instead of staying there for a
      // fresh adoption. Clearing WiFi too means ANY future boot - not
      // just this immediate one - lands in setup mode until someone
      // actually re-configures it.
      config.wifi_ssid = "";
      config.wifi_pass = "";
      saveConfig();
      http.end();
    ...
    if (wifiLostAt == 0) {
      ...
      if (config.is_adopted) {
        ...
        if (millis() - wifiLostAt >= WIFI_DISCONNECTED_REBOOT_MS) {
          ...
          // already found and fixed for the ESP8266 sibling's setup-mode
          // reboot. Explicitly turning the radio off before restarting
          // forces a real re-init of it on the way back up, instead of
          // whatever bad state survived a plain CPU-level restart.
