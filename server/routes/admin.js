...
router.post('/network', adminAuth, (req, res) => {
  try {
    ...
    // regenerated by cloud-init on every boot, so anything we wrote there was
    // silently wiped back to DHCP on next reboot. A file sorting after it wins
    // the netplan merge on matching keys without needing cloud-init involved.
