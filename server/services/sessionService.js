...
function createSession(mac, ip, minutes, expirationMinutes) {
...
function addTimeToSession(mac, minutes, expirationMinutes) {
  ...
  // Option A — each coin resets expiration
...
function resumeSession(voucherCode) {
  ...
  // Check if hard expiration passed
  const now = new Date();
...
function expireSession(voucherCode) {
  db.prepare(
