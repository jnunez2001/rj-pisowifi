...
- **What this does *not* fully solve:** if the device running this app is physically plugged into a port that Configure needs to restructure (a real, common topology, not an edge case), there is still a brief window, now single-digit API calls instead of an entire run, where that port is disconnected before landing on its new bridge. On real hardware this is likely to be fast enough that an already-established TCP connection survives it, but that isn't guaranteed. The durable fix, not yet built: warn the admin before they click Configure if their own request appears to be arriving through a port about to be restructured, and/or process that specific port first, before anything else, to minimize its own exposure window further. Noted here as a real follow-up, not closed out by this fix alone.
- **Verification status:** field-tested against a fake router simulating the exact real factory-default state from tonight's failure (`ether2`/`ether3`/`ether4`/`ether5` all pre-existing members of one factory bridge) - confirmed the generated plan frees `ether2` only 2 steps before its own bridge is created (previously it would have been the very first of many separate cleanup steps, with the actual bridge not built until much later), confirmed `ether2` is freed exactly once despite being referenced by two different lanes, and confirmed a full `apply()` run against that fake router succeeds end to end (39 steps, all ok, the fake bridge ends up correctly empty as every port migrates to its new home). Not yet re-run against the real hEX E50UG that hit this bug tonight - please confirm on the actual hardware once ether2 is manually restored to the factory bridge as a starting point again.
